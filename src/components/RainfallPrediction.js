...
const RainfallPrediction = () => {
  ...
  const loadFeatures = async () => {
    try {
      const res = await fetch('http://16.171.142.225/rainfall/features');
  ...
  const loadGraphs = async () => {
    ...
    try {
      const res = await fetch('http://16.171.142.225/rainfall/graphs');
      const data = await res.json();
  ...
  const handlePredict = async () => {
    ...
    setPrediction(null);
    try {
      const res = await fetch('http://16.171.142.225/rainfall/predict', {
        method: 'POST',
