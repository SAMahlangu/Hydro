...
const BillingForecast = () => {
  ...
  useEffect(() => {
    const fetchModels = async () => {
      try {
        const res = await fetch('http://localhost:5000/billing-forecast/models');
  ...
  const predict = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('http://localhost:5000/billing-forecast/predict', {
        method: 'POST',
  ...
  const loadGraphs = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/billing-forecast/graphs');
      const data = await res.json();
