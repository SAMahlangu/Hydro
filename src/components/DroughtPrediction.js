...
const DroughtPrediction = () => {
  ...
  const loadChartData = async () => {
    try {
      ...
      const timeSeriesResponse = await fetch('http://16.171.142.225/drought-timeseries-data');
      ...
      // Load variable data
      const variableResponse = await fetch('http://16.171.142.225/drought-variable-data');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://16.171.142.225/predict-drought', {
        method: 'POST',
        headers: {
