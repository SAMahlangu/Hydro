...
const LeakageDetection = () => {
  ...
  useEffect(() => {
    const fetchModels = async () => {
      try {
        const res = await fetch('http://16.171.142.225/leakage/models');
  ...
  const loadSummary = async () => {
    try {
      const res = await fetch('http://16.171.142.225/leakage/summary');
      const data = await res.json();
      setSummary(data);
    } catch (e) {
      console.error(e);
  ...
  const handlePredict = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('http://16.171.142.225/leakage/predict', {
        method: 'POST',
