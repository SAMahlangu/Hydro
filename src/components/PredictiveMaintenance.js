...
const PredictiveMaintenance = () => {
  ...
  const loadFeatures = async () => {
    try {
      const res = await fetch('http://localhost:5000/predictive-maintenance/features');
  ...
  const loadGraphs = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/predictive-maintenance/graphs');
      const data = await res.json();
  ...
  const handlePredict = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('http://localhost:5000/predictive-maintenance/predict', {
        method: 'POST',
